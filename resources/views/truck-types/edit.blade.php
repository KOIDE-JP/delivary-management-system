        ...
        <div class="max-w-2xl mx-auto">
            <div class="form-card gradient-border rounded-2xl p-6 sm:p-8 md:p-10 bg-white shadow-lg border border-gray-200">
                <div class="relative z-10">
                    <h2 class="text-2xl font-bold mb-4">{{ __('layouts.edit_truck_type') }}</h2>

                    <form action="{{ route('truck-types.update', $truckType->id) }}" method="POST">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="mb-6">
                            ...
                            <select name="status"
                                class="w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-blue-400 focus:border-blue-400">
                                <option value="1" {{ old('status', $truckType->status) == 1 ? 'selected' : '' }}>{{ __('layouts.active') }}</option>
                                <option value="0" {{ old('status', $truckType->status) == 0 ? 'selected' : '' }}>{{ __('layouts.inactive') }}</option>
                            </select>
                            @error('status')
                                <span class="text-red-600 text-sm">{{ $message }}</span>
                            @enderror
